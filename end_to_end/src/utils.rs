...
use crate::mpt::{apply_diffs, insert_mpt, Mpt, trim};
...
pub async fn get_block_hashes(block_number: U64, provider: &Provider<Http>) -> anyhow::Result<BlockHashes> {
...
pub async fn gather_witness(tx: TxHash, provider: &Provider<Http>) -> anyhow::Result<Vec<TxnProofGenIR>> {
    let tx = provider
        .get_transaction(tx)
        .await?
