...
use crate::utils::{EMPTY_HASH, keccak};
            ...
            2 => match a[0][0] >> 4 {
                ...
                1 => {
                    ...
                    let mut ext_prefix = if a[0].len() > 1 {
                        ...
                        Nibbles::from_h256_be(H256::zero())
                        // Nibbles {
                        //     count: 0,
                        //     //packed: U512::zero(),
                        //     packed: U256::zero(),
...
pub fn apply_diffs(
        ...
        if !diff.post.contains_key(addr) {
            ...
            for (&k, &v) in &old.storage.clone().unwrap_or_default() {
                if !new.storage.clone().unwrap_or_default().contains_key(&k) {
                    ...
                    trie.insert(tokk(k), rlp::encode(&w.into_uint()).to_vec()).unwrap();
                }
            }
            for (&k, v) in &new.storage.clone().unwrap_or_default() {
                if !old.storage.clone().unwrap_or_default().contains_key(&k) {
                    trie.insert(tokk(k), rlp::encode(&v.into_uint()).to_vec()).unwrap();
                }
            }
    ...
    for (addr, acc) in &diff.post {
        if !diff.pre.contains_key(addr) {
            ...
            mpt.insert(tok(addr), rlp::encode(&account).to_vec()).unwrap();
        } else {
            let old = mpt
                .get(tok(addr))
