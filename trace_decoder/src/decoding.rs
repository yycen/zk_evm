...
impl ProcessedBlockTrace {
    ...
    fn get_trie_trace(trie: &HashedPartialTrie, k: &Nibbles) -> Vec<TrieSegment> {
    ...
    fn node_deletion_resulted_in_a_branch_collapse(
        old_path: &[TrieSegment],
        new_path: &[TrieSegment],
    ) -> bool {
        ...
        if matches!(&old_path[seg_idx], TrieSegment::Branch(_)) {
            return false;
        }
