        ...
        for h_addr in accounts_with_storage {
            if !storage_tries.contains_key(h_addr) {
                let trie = state_accounts_with_no_accesses_but_storage_tries
                    .get(h_addr)
                    .map(|s_root| {
                        println!("CREATING SINGLE HASH TRIE FOR {:x}!!!!", h_addr);
                        HashedPartialTrie::new(Node::Hash(*s_root))
                    })
                    .unwrap_or_default();
...
fn create_minimal_storage_partial_tries<'a>(
    ...
    accesses_per_account
        .map(|(h_addr, mem_accesses)| {
            ...
            let storage_slots_to_not_hash = mem_accesses
                .iter()
                .cloned()
                .chain(
            ...
            println!(
                "Storage slots to avoid hashing for {}: {:#?}",
                h_addr, storage_slots_to_not_hash
            );
